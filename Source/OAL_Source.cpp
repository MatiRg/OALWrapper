...
int cOAL_Source::BindData(cOAL_Sample* apSample)
{
    ...
    if (apSample == nullptr || apSample->GetStatus() == false)
        return -1;
...
void cOAL_Source::UpdateFiltering(unsigned int)
{
    if (!gpDevice->IsEFXActive())
        return;
