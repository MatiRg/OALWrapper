...
{
    std::string sTemp;
    size_t needed = wcstombs(nullptr, &asWString[0], asWString.length());
    sTemp.resize(needed);
...
{
    std::wstring wsTemp;
    size_t needed = mbstowcs(nullptr, &asString[0], asString.length());
...
void ClearALErrors(const std::string&)
{
...
bool CheckALErrors(const std::string&, const std::string&)
{
...
void ClearALCErrors(const std::string&)
{
    ALCcontext* ctx = alcGetCurrentContext();
    if (ctx == nullptr)
        return;
    ...
    //	OAL_Log(2, "%s ClearALCErrors raised %d\n", asFunction.c_str(), eStatus );
}
