...
static size_t OAL_OggBufferRead(void* dest, size_t eltSize, size_t nelts, OAL_OggMemoryFile* src)
{
    ...
    if ((src->pos + static_cast<ogg_int64_t>(len)) > src->size)
    {
...
static int OAL_OggBufferClose(OAL_OggMemoryFile*)
{
    return 0;
}
...
bool cOAL_OggSample::CreateFromFile(const std::wstring& asFilename)
{
    ...
    if ((lOpenResult = ov_open_callbacks(fileHandle, &ovFileHandle, nullptr, 0, OV_CALLBACKS_DEFAULT)) < 0)
    {
        fclose(fileHandle);
        mbStatus = false;
        return false;
...
bool cOAL_OggSample::CreateFromBuffer(const void* apBuffer, size_t aSize)
{
    ...
    OggVorbis_File ovFileHandle;
    if ((lOpenResult = ov_open_callbacks(&fakeFile, &ovFileHandle, nullptr, 0, OAL_CALLBACKS_BUFFER)) < 0)
    {
        mbStatus = false;
        return false;
    }
