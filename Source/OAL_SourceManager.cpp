...
cOAL_SourceManager::cOAL_SourceManager()
    : mbManageVoices(true), mlNumOfVoices(0), mlAvailableVoices(0), mpStreamListMutex(nullptr), mpUpdaterThread(nullptr), mlThreadWaitTime(0), mbUseThreading(false)
{
}
...
bool cOAL_SourceManager::Initialize(bool abManageVoices,
                                    ...
                                    int alEFXSends)
{
    ...
    while (mlNumOfVoices < alNumSourcesHint)
    {
        ...
        if (!AL_ERROR_OCCURED)
        {
            LogMsg("",
                   ...
                   eOAL_LogMsg_Info,
                   "Source Object successfully created\n");
            ++mlNumOfVoices;
        }
        else
            break;
    ...
    LogMsg("",
           eOAL_LogVerbose_Medium,
           eOAL_LogMsg_Info,
           "Created %d Low Level Sources, %d hinted\n",
           mlNumOfVoices,
           alNumSourcesHint);
    ...
    LogMsg(
        "", eOAL_LogVerbose_Medium, eOAL_LogMsg_Info, "Done creating Sources\n");
    ...
    if (mbUseThreading)
    {
        ...
        // This converts the desired frequency in aInput to amount of
        // milliseconds to wait. Note that this is an int value, so any freq
        // above 1000 will turn mlThreadWaitTime to 0;
        mlThreadWaitTime = 1000 / alUpdateFreq;
        mpUpdaterThread = SDL_CreateThread(UpdaterThread, "OAL Updater", nullptr);
...
void cOAL_SourceManager::Destroy()
{
    ...
    // Delete sources
    LogMsg(
        "", eOAL_LogVerbose_Medium, eOAL_LogMsg_Info, "Deleting Sources...\n");
    // hpl::Log("Sources: %d\n",mpvSources->size());
    tSourceVecIt it;
    ...
    LogMsg("",
           ...
           eOAL_LogMsg_Info,
           "",
           "Deleting active streams list...\n");
    mlstStreamingSources.clear();
}

//-----------------------------------------------------------------------------------

cOAL_Source* cOAL_SourceManager::GetSource(int alSourceHandle,
                                           bool abSkipRefCountCheck)
{
    //////////////////////////////////
    ...
    //////////////////////////////////
    // Check so that ref count is valid. (this will invalidate sources have been
    // changed)
...
cOAL_Source* cOAL_SourceManager::GetAvailableSource(unsigned int alPriority,
                                                    int alNumOfVoices)
{
    cOAL_Source* pSource;
    int lSourceHandle = -1;
    ...
    while (mlAvailableVoices < alNumOfVoices)
    {
        ...
        if (lLowestPrioSource != -1)
        {
            ...
            pSource->Lock();
            pSource->Stop();
            pSource->IncRefCount(); // Do this to make sure the source is
                                    // invalidated!
            pSource->Unlock();
        }
        //////////////////////
